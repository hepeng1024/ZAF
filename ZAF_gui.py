#!/usr/bin/env python3
"""
Tkinter GUI for ZAF.py.

Run with:
  conda activate zaf
  python ZAF_gui.py

The GUI is a thin wrapper around ZAF.py, so the original command
...
try:
    ...
    raise SystemExit(
        ...
        "Run this GUI with the same environment used for ZAF.py, for example:\n"
        "  conda activate zaf\n"
        "  python ZAF_gui.py"
    )
...
class SampleTiltSimulator(tk.Toplevel):
    ...
    def __init__(
        ...
        self.scale_bar_pixels_var = StringVar(
            value=(
                f"{context.scale_bar_pixels:.1f}"
                if context.scale_bar_pixels is not None
                else ""
            )
        )
        self.scale_bar_value_var = StringVar(
            value=(
                f"{context.scale_bar_value_inv_nm:g}"
                if context.scale_bar_value_inv_nm is not None
                else ""
            )
        )
        ...
        if context.crystal_structure == "HCP":
            (
                self.base_crystal,
                self.base_crystal_edges,
            ) = self.create_crysdis_hcp_lattice(
                size=5.6,
                hcp_c_over_a=context.hcp_c_over_a,
            )
        else:
            self.base_crystal = self.create_structure_lattice(
                context.crystal_structure,
                size=5.6,
                hcp_c_over_a=context.hcp_c_over_a,
            )
            self.base_crystal_edges = np.zeros((3, 0), dtype=float)
        self.base_reciprocal = self.create_reciprocal_lattice(
            context.crystal_structure, size=4.6, hcp_c_over_a=context.hcp_c_over_a
        )
        self.diffraction_max_index = 20
        self.diffraction_hkl = self.generate_diffraction_hkl(
            max_index=self.diffraction_max_index
        )
    ...
    def _build_diffraction_parameter_panel(self, parent: ttk.LabelFrame) -> None:
        ...
        ttk.Label(parent, text="Scale line px").grid(row=2, column=2, sticky="w", padx=(0, 5), pady=(0, 3))
        ...
        ttk.Label(parent, text="Printed bar 1/nm").grid(row=3, column=0, sticky="w", padx=(0, 5), pady=(0, 3))
        ...
        note = ttk.Label(
            parent,
            text="The line length is detected; enter the printed value such as 5 manually.",
            anchor="w",
        )
    ...
    def _initial_lattice_parameter_nm(self) -> float:
        if self.context.expected_lattice_parameter_nm is not None:
            return self.context.expected_lattice_parameter_nm
        estimated = getattr(self.context.match, "estimated_lattice_nm", None)
        if estimated is not None and float(estimated) > 0:
            return float(estimated)
        return ZAF.DEFAULT_LATTICE_PARAMETER_NM[self.context.crystal_structure]
    ...
    def generate_diffraction_hkl(self, max_index: int) -> np.ndarray:
        ...
        for h in range(-max_index, max_index + 1):
            for k in range(-max_index, max_index + 1):
                for l in range(-max_index, max_index + 1):
                    if h == k == l == 0:
                        continue
                    reciprocal = ZAF.reciprocal_cartesian(
                        (h, k, l),
                        self.context.crystal_structure,
                        self.context.hcp_c_over_a,
                    )
                    if float(np.dot(reciprocal, reciprocal)) > max_index * max_index:
                        continue
                    if ZAF.reflection_allowed(
                        self.context.crystal_structure, h, k, l
                    ):
    ...
    def _ensure_diffraction_hkl(self, max_index: int) -> None:
        if max_index == self.diffraction_max_index:
            return
        self.diffraction_hkl = self.generate_diffraction_hkl(max_index=max_index)
    ...
    def _initialize_scale_bar_detection(self) -> None:
        if self.context.image_path is None or self.scale_bar_pixels_var.get().strip():
            return
        with contextlib.suppress(Exception):
            result = ZAF.detect_scale_bar_pixels(Path(self.context.image_path))
    ...
    def _write_output(self) -> None:
        ...
        lines = [
            ...
            f"a={self.lattice_parameter_var.get().strip() or str(ZAF.DEFAULT_LATTICE_PARAMETER_NM[self.context.crystal_structure])} nm, "
    ...
    def create_structure_lattice(
        ...
        if crystal_structure == "FCC":
            return cls.create_fcc_lattice(size)
        if crystal_structure == "BCC":
            corners = cls.create_fcc_lattice(size)[:, :8]
            return np.column_stack((corners, np.zeros(3, dtype=float)))
        points, _edges = cls.create_crysdis_hcp_lattice(size, hcp_c_over_a)
        return points

    @staticmethod
    def create_crysdis_hcp_lattice(
        size: float,
        hcp_c_over_a: float,
    ) -> tuple[np.ndarray, np.ndarray]:
        """Reproduce the built-in primitive HCP cell used by CrysDiS.py.

        CrysDiS defines a 120-degree primitive cell with Mg basis sites at
        (0, 0, 0) and (2/3, 1/3, 1/2), repeats the origin site on cell
        boundaries, centers the cell, and normalizes it by its largest span.
        """
        lattice_rows = ZAF.direct_lattice_matrix(
            "HCP", hcp_c_over_a
        ).T
        corners_fractional = np.asarray(
            list(itertools.product((0.0, 1.0), repeat=3)), dtype=float
        )
        corners_cartesian = corners_fractional @ lattice_rows
        center = np.asarray((0.5, 0.5, 0.5), dtype=float) @ lattice_rows
        edge_norm = float(np.max(np.linalg.norm(lattice_rows, axis=1)))
        bounding_span = float(np.max(np.ptp(corners_cartesian, axis=0)))
        normalization = max(edge_norm, bounding_span, 1e-8)

        def display_position(fractional: np.ndarray) -> np.ndarray:
            return ((fractional @ lattice_rows - center) / normalization) * (
                2.0 * size
            )

        atom_fractional = [
            *corners_fractional,
            np.asarray((2.0 / 3.0, 1.0 / 3.0, 0.5), dtype=float),
        ]
        atom_points = np.asarray(
            [display_position(fractional) for fractional in atom_fractional],
            dtype=float,
        ).T

        edge_points: list[np.ndarray] = []
        for index, start in enumerate(corners_fractional):
            for end in corners_fractional[index + 1 :]:
                if np.count_nonzero(np.abs(start - end) > 1e-9) == 1:
                    edge_points.extend(
                        (display_position(start), display_position(end))
                    )
        return atom_points, np.asarray(edge_points, dtype=float).T

    @staticmethod
    def create_reciprocal_lattice(
        crystal_structure: str,
        size: float,
        hcp_c_over_a: float,
    ) -> np.ndarray:
        indices: list[tuple[int, int, int]] = [(0, 0, 0)]
        for h, k, l in itertools.product(range(-2, 3), repeat=3):
            if h == k == l == 0:
                continue
            if ZAF.reflection_allowed(crystal_structure, h, k, l):
                indices.append((h, k, l))
        points = ZAF.reciprocal_lattice_matrix(
            crystal_structure, hcp_c_over_a
        ) @ np.asarray(indices, dtype=float).T
        norms = np.linalg.norm(points, axis=0)
        nonzero = norms[norms > 1e-9]
        if nonzero.size:
            points *= size / float(np.percentile(nonzero, 70))
        return points
    ...
    def draw_structure_lattice_on_ax(
        self,
        axis: object,
        rotated_points: np.ndarray,
        rotated_edges: np.ndarray | None = None,
    ) -> None:
        plot_x, plot_y, plot_z = self.screen_plot_coordinates(rotated_points)
        color = {"FCC": "#0ABAB5", "BCC": "#E67E22", "HCP": "#9B59B6"}[
            self.context.crystal_structure
        ]
        axis.scatter(
            plot_x,
        ...
        if rotated_edges is not None and rotated_edges.shape[1] >= 2:
            edge_x, edge_y, edge_z = self.screen_plot_coordinates(rotated_edges)
            for index in range(0, rotated_edges.shape[1] - 1, 2):
                axis.plot(
                    [edge_x[index], edge_x[index + 1]],
                    [edge_y[index], edge_y[index + 1]],
                    [edge_z[index], edge_z[index + 1]],
                    color="#6B7280",
                    linewidth=1.1,
                    alpha=0.85,
                )

    def draw_reciprocal_lattice_on_ax(
        self, axis: object, rotated_points: np.ndarray
    ) -> None:
        plot_x, plot_y, plot_z = self.screen_plot_coordinates(rotated_points)
        radii = np.linalg.norm(rotated_points, axis=0)
        sizes = 55.0 + 100.0 * np.exp(-0.16 * radii * radii)
        axis.scatter(
            plot_x,
            plot_y,
            plot_z,
    ...
    def _crystal_display_points(self, crystal_points: np.ndarray) -> np.ndarray:
        ...
        sample_rotated_points = ZAF.rotate_zero_holder_in_plane(
            zero_holder_points,
            self.sample_ccw_deg,
        )
        tilted_points = ZAF.holder_rotation(
            self.alpha_deg,
            self.beta_deg,
    ...
    def _current_reciprocal_vectors_in_holder(self, lattice_parameter_nm: float) -> np.ndarray:
        g_crystal = (
            ZAF.reciprocal_lattice_matrix(
                self.context.crystal_structure, self.context.hcp_c_over_a
            )
            @ self.diffraction_hkl
    ...
    def _update_diffraction_pattern(self) -> None:
        ...
        if relrod_method.startswith("gauss"):
            ...
            relrod_label = "sinc"
        reciprocal_indices = ZAF.reciprocal_lattice_matrix(
            self.context.crystal_structure, self.context.hcp_c_over_a
        ) @ self.diffraction_hkl
        reciprocal_norm2 = np.sum(reciprocal_indices * reciprocal_indices, axis=0)
        form_factor = np.exp(-0.010 * reciprocal_norm2)
    ...
    def update_plot(self) -> None:
        ...
        self.draw_structure_lattice_on_ax(
            self.real_ax,
            self._crystal_display_points(self.base_crystal),
            self._crystal_display_points(self.base_crystal_edges)
            if self.base_crystal_edges.size
            else None,
        )
        self.draw_reciprocal_lattice_on_ax(
            self.recip_ax, self._crystal_display_points(self.base_reciprocal)
        )
    ...
    def _pole_points(self) -> list[ZAF.ZoneAxisMapPoint]:
        points = ZAF.sample_rotation_map_points(
        ...
        if self.context.map_reachable_only:
            points = ZAF.filter_reachable_zone_axis_map_points(
                points,
                self.context.crystal_to_zero_holder,  # type: ignore[arg-type]
                self.context.holder_order,
    ...
    def _update_pole_figure(self) -> None:
        ...
        for point in points:
            ...
            fill = ZAF.ZONE_FAMILY_COLORS.get(point.family, (80, 80, 80))
            outline = ZAF.zone_outline_color(point.zone)
            ...
            label = ZAF.format_zone_direction(
    ...
    def _pole_axis_range(self, points: Sequence[ZAF.ZoneAxisMapPoint]) -> float:
        values = [
...
class SampleRotationSimulator(tk.Toplevel):
    ...
    def _dynamic_map_points(self, ccw_deg: float) -> list[ZAF.ZoneAxisMapPoint]:
        points = ZAF.sample_rotation_map_points(
            self.context.match,  # type: ignore[arg-type]
            self.context.crystal_to_zero_holder,  # type: ignore[arg-type]
            ccw_deg=ccw_deg,
    ...
    def _compute_dynamic_map_ranges(self) -> tuple[tuple[float, float], tuple[float, float]]:
        ...
        return (
            self._expand_axis_range(ZAF.padded_range(alpha_values)),
            self._expand_axis_range(ZAF.padded_range(beta_values)),
        )
    ...
    def _update_reachable_output(self) -> None:
        ccw_deg = self.current_ccw_deg()
        rows = ZAF.sample_rotation_reachable_rows(
            self.context.match,  # type: ignore[arg-type]
            self.context.crystal_to_zero_holder,  # type: ignore[arg-type]
            ccw_deg=ccw_deg,
            target_families=self.context.target_families,
...
class ZAFGUI(tk.Tk):
    def __init__(self) -> None:
        ...
        self.landing_text_photos: dict[str, ImageTk.PhotoImage] = {}
    ...
    def _build_variables(self) -> None:
        defaults = {
            ...
            "max_index": str(ZAF.DEFAULT_REFLECTION_MAX_INDEX),
            "max_g_norm": str(ZAF.DEFAULT_REFLECTION_MAX_G_NORM),
            "tolerance_fraction": "0.18",
            "scale_bar_value_inv_nm": "",
            "expected_lattice_parameter_nm": "",
            "hcp_c_over_a": f"{ZAF.DEFAULT_HCP_C_OVER_A:.6f}",
        ...
        self.bool_vars = {
            ...
            "hcp_four_index": BooleanVar(value=True),
        }
        all_families = dict.fromkeys(
            (*ZAF.SUPPORTED_ZONE_FAMILIES, *ZAF.HCP_ZONE_FAMILIES)
        )
        self.family_vars = {
            family: BooleanVar(
                value=(
                    family in DEFAULT_SELECTED_TARGET_FAMILIES
                    or family in DEFAULT_SELECTED_HCP_TARGET_FAMILIES
                )
            )
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Button(header, text="Introduction / Help", command=self.show_introduction).grid(row=0, column=2, sticky="e")
    ...
    def _build_basic_tab(self, parent: ttk.Frame) -> None:
        ...
        self.current_zone_hint = ttk.Label(
            indexing, text="Example: 1,0,0. \nLeave blank to auto-detect."
        )
        self.current_zone_hint.grid(
            row=1, column=1, sticky="w", pady=(3, 0)
        )
        if self.crystal_structure == "HCP":
            hcp_options = ttk.Frame(indexing)
            hcp_options.grid(row=2, column=0, columnspan=2, sticky="ew", pady=(8, 0))
            hcp_options.columnconfigure(3, weight=1)
            ttk.Checkbutton(
                hcp_options,
                text="Use four-index [u v t w] zone notation",
        ...
        for idx, family in enumerate(structure_families):
            ...
            checkbutton.grid(
                row=idx // family_columns,
                column=idx % family_columns,
                sticky="w",
                padx=(0, 16),
                pady=2,
            )
            self.family_checkbuttons[family] = checkbutton
        option_row = (
            len(structure_families) + family_columns - 1
        ) // family_columns
        ttk.Checkbutton(
            targets,
            text="Report opposite directions separately",
            variable=self.bool_vars["include_opposites"],
        ).grid(row=option_row, column=0, columnspan=family_columns, sticky="w", pady=(8, 0))
        ttk.Checkbutton(
            targets,
    ...
    def _refresh_hcp_notation(self) -> None:
        ...
        if hasattr(self, "current_zone_hint"):
            example = "2-1-10" if four_index else "1,0,0"
            self.current_zone_hint.configure(
    ...
    def show_introduction(self) -> None:
        ...
        ttk.Label(
            content,
            text=f"{self.crystal_structure} Zone-Axis Finder: Introduction",
            style="Title.TLabel",
        ).grid(
        ...
        row = self._intro_section(
            ...
            f"This GUI starts from one {self.crystal_structure} zone-axis diffraction pattern and the current double-tilt holder angles. "
            f"It detects the diffraction spots, compares them with analytic {self.crystal_structure} zone-axis reference "
        ...
        row = self._intro_section(
            ...
            f"screen-view tilt window with a 3D tilt view, real-space {self.crystal_structure} crystal view, and reciprocal-space lattice view, "
            ...
            "The real and reciprocal lattices are mapped through the indexed crystal orientation, so they follow the same "
            ...
            f"Max hkl controls how many {self.crystal_structure} reciprocal-lattice reflections are considered, the relrod model chooses either a finite-thickness "
            ...
            "Both relrod models are still multiplied by the same high-index form-factor falloff. The lattice parameter starts from a structure-specific fallback value. "
        ...
        row = self._intro_section(
            ...
            f"Known current zone: optional. Leave it blank to let the program choose among the supported {self.crystal_structure} zone axes. "
            "Enter values like 1,0,0 or 1 1 0 if you already know the present zone and want to force that assignment. "
            "For HCP, four-index notation is the default, accepts compact directions such as [2-1-10], and c/a controls the non-cubic geometry.\n\n"
            "Target families: choose which zone-axis families to calculate. FCC and BCC use the 26 primitive three-index families with h+k+l <= 8. "
            "HCP uses the nonduplicated four-index catalog <0001>, <2-1-10>, <10-10>, <10-11>, <10-12>, <11-23>, <21-30>, and <40-43>.\n\n"
        ...
        row = self._intro_section(
            ...
            f"Max index: largest absolute h, k, or l included in the analytic {self.crystal_structure} reference pattern. The expanded default "
            "includes the higher-order spots needed to distinguish families such as <106>, <205>, and <304>.\n\n"
            "Max |g|: largest reciprocal-vector length included in the reference pattern. High-index families can be "
            "indistinguishable when their higher-order spots fall outside the input image.\n\n"
            ...
            "Printed bar 1/nm: optional reciprocal-space number printed beside the image scale bar, such as 5. The program "
            "measures the bright bar line automatically but does not OCR this number, so enter it manually. Candidate lattice "
            "parameters are then shown in the reference scores.\n\n"
            "Known lattice nm: optional known lattice parameter a. When supplied with the printed scale-bar value, it is "
            "used to resolve close geometric matches. Leave it blank for materials whose lattice parameter is unknown.\n\n"
            "Ambiguous matches automatically receive a second, more sensitive spot-detection pass. If the leading candidates "
            "still remain too close, the output warns that the indexing is ambiguous instead of presenting it as confident.\n\n"
    ...
    def _make_landing_photo(
        ...
        """Composite transparent crystal art onto the active Tk frame gray."""
        try:
            image = Image.open(path).convert("RGBA")
            image.thumbnail(max_size, Image.Resampling.LANCZOS)
            style_color = ttk.Style(self).lookup("TFrame", "background")
            background_color = style_color or str(self.cget("background"))
            red, green, blue = self.winfo_rgb(background_color)
            background = Image.new(
                "RGBA",
                image.size,
                (red // 256, green // 256, blue // 256, 255),
            )
    ...
    def _make_landing_text_photo(
        ...
        """Render large landing text as an image, following CrysDiS tkinter."""
        try:
            font_name = "DejaVuSans-Bold.ttf" if bold else "DejaVuSans.ttf"
            font_path = (
                Path(matplotlib.get_data_path()) / "fonts" / "ttf" / font_name
            )
            try:
            ...
            width = max(1, bounds[2] - bounds[0] + 2 * xpad)
            height = max(1, bounds[3] - bounds[1] + 2 * ypad)
            image = Image.new("RGBA", (width, height), (0, 0, 0, 0))
            draw = ImageDraw.Draw(image)
            draw.text(
                (xpad - bounds[0], ypad - bounds[1]),
                text,
                font=font,
                fill=color,
            )
    ...
    def build_settings(self) -> dict[str, object]:
        ...
        scale_bar_value_inv_nm = self._optional_positive_float_value(
            "scale_bar_value_inv_nm",
            "Printed scale-bar value",
        )
        expected_lattice_parameter_nm = self._optional_positive_float_value(
            "expected_lattice_parameter_nm",
            "Known lattice parameter",
        )
        if expected_lattice_parameter_nm is not None and scale_bar_value_inv_nm is None:
            raise ValueError(
                "Known lattice parameter requires the printed scale-bar value in 1/nm."
            )
        ...
        return {
            ...
            "max_index": self._int_value(
                "max_index",
                ZAF.DEFAULT_REFLECTION_MAX_INDEX,
            ),
            "max_g_norm": self._float_value(
                "max_g_norm",
                ZAF.DEFAULT_REFLECTION_MAX_G_NORM,
            ),
    ...
    def _optional_positive_float_value(
        ...
        value = self._optional_float_value(key)
    ...
    def _run_gui_analysis(self, settings: dict[str, object], previews: PreviewResult) -> PreviewResult:
        ...
        if settings["rotate_pattern_180"]:
            rotated_best = ZAF.rotate_match_in_plane(
                best,
                180.0,
                peaks,
                rgb.size,
            ...
            best = rotated_best
        crystal_to_zero_holder, _image_axes, _holder_axes = ZAF.orientation_from_match(
            best,
        ...
        if settings["map_reachable_only"]:
            map_points = ZAF.filter_reachable_zone_axis_map_points(
                map_points,
                crystal_to_zero_holder,
        ...
        zone_map_image = ZAF.zone_axis_map_image(
            map_points,
            alpha_limits=settings["alpha_limits"],  # type: ignore[arg-type]
            beta_limits=settings["beta_limits"],  # type: ignore[arg-type]
        ...
        if settings["scale_bar_value_inv_nm"] is not None and scale_bar_pixels is None:
            print(
                "\nWarning: the scale-bar line could not be detected, so lattice "
                "calibration was not applied."
            )
        ZAF.print_match_summary(
            best,
            all_results,
            diagnostics=indexing.diagnostics,
        )
        if settings["rotate_pattern_180"]:
        ...
        fitted_image = ZAF.fitted_diffraction_image(
            rgb,
            best,
            show_labels=settings["show_labels"],  # type: ignore[arg-type]
            draw_guides=settings["show_kikuchi_guides"],  # type: ignore[arg-type]
        ...
        if "target_csv" in export_files:
            ...
            ZAF.write_targets_csv(path, rows)
            written.append(("target angles", path))
        if "indexed_spots" in export_files:
            path = Path(f"{prefix}_indexed_spots.csv")
            ZAF.write_indexed_spots_csv(path, best, peaks)
            written.append(("indexed spots", path))
        if "predicted_pattern" in export_files:
            path = Path(f"{prefix}_predicted_zone_pattern.png")
            if predicted_image is None:  # pragma: no cover - guarded above.
                raise RuntimeError("Predicted-pattern export was not rendered.")
